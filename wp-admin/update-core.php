...
function core_upgrade_preamble()
{
    ...
    if ($updates && (count($updates) > 1 || $updates[0]->response != 'latest')) {
        ...
        echo '<p>' . sprintf(
            __('<a href="%s">Learn more about WordPress %s</a>.'),
...
function list_plugin_updates()
{
    ...
    if (!isset($core_updates[0]->response) || 'latest' == $core_updates[0]->response || 'development' == $core_updates[0]->response || version_compare(
        ...
        <table class="widefat updates-table" id="update-plugins-table">
            ...
            <tbody class="plugins">
                ...
                foreach ((array)$plugins as $plugin_file => $plugin_data) {
                    ...
                    if (isset($plugin_data->update->tested) && version_compare(
                        $plugin_data->update->tested,
                            $cur_wp_version,
                        '>='
                    )) {
                        $compat = '<br />' . sprintf(
                            __('Compatibility with WordPress %1$s: 100%% (according to its author)'),
                                $cur_wp_version
                        );
                    ...
                    if ($core_update_version) {
                        if (isset($plugin_data->update->tested) && version_compare(
                            ...
                            $compat .= '<br />' . sprintf(
                                __('Compatibility with WordPress %1$s: Unknown'),
                                    $core_update_version
                            );
                        }
                    }
