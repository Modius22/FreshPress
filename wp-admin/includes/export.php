...
function export_wp($args = array())
{
    ...
    if ('post' == $args['content'] || 'page' == $args['content'] || 'attachment' == $args['content']) {
        ...
        if ($args['end_date']) {
            $where .= $wpdb->prepare(" AND {$wpdb->posts}.post_date < %s",
                date('Y-m-d', strtotime('+1 month', strtotime($args['end_date']))));
        }
    }
                ...
                while ($next_posts = array_splice($post_ids, 0, 20)) {
                    ...
                    foreach ($posts as $post) {
                        ...
                        <item>
                            ...
                            <?php $postmeta = $wpdb->get_results($wpdb->prepare("SELECT * FROM $wpdb->postmeta WHERE post_id = %d",
                                $post->ID));
                            ...
                            foreach ($comments as $c) : ?>
                                <wp:comment>
                                    ...
                                    <?php $c_meta = $wpdb->get_results($wpdb->prepare("SELECT * FROM $wpdb->commentmeta WHERE comment_id = %d",
                                        $c->comment_ID));
                                    foreach ($c_meta as $meta) :
                                         ...
                                         * @param object $meta Current meta object.
                                         */
                                        if (apply_filters('wxr_export_skip_commentmeta', false, $meta->meta_key,
                                            $meta)) {
                                            continue;
                                        }
                                        ?>
