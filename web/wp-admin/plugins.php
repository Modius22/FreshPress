    ...
    switch ($action) {
        case 'activate':
            ...
            if (isset($_GET['from']) && 'import' == $_GET['from']) {
                wp_redirect(self_admin_url("import.php?import=" . str_replace(
                    '-importer',
                    '',
                        dirname($plugin)
            ...
            if (!isset($_REQUEST['verify-delete'])) {
                ...
                foreach ($plugin_info as $plugin) {
                    ...
                    <form method="post" action="<?php echo esc_url($_SERVER['REQUEST_URI']); ?>"
                        ...
                        <?php submit_button(
                            $data_to_delete ? __('Yes, delete these files and data') : __('Yes, delete these files'),
                            '',
                            'submit',
                            false
                        ); ?>
...
<?php if (isset($_GET['error'])) :
    ...
    if (isset($_GET['main'])) {
        ...
        $errmsg = sprintf(
            __('The plugin generated %d characters of <strong>unexpected output</strong> during activation. If you notice &#8220;headers already sent&#8221; messages, problems with syndication feeds or other issues, try deactivating or removing this plugin.'),
            $_GET['charsout']
        );
    } else {
    ...
    <div id="message" class="error"><p><?php echo $errmsg; ?></p>
        <?php
        if (!isset($_GET['main']) && !isset($_GET['charsout']) && wp_verify_nonce(
        $_GET['_error_nonce'],
