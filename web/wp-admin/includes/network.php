...
function allow_subdomain_install()
{
    ...
    if (parse_url(
        get_option('home'),
            PHP_URL_PATH
    ) || 'localhost' == $domain || preg_match('|^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$|', $domain)) {
        return false;
    }
...
function network_step1($errors = false)
{
    ...
    echo ' <strong>' . __('The main site in a sub-directory install will need to use a modified permalink structure, potentially breaking existing links.') . '</strong>'; ?></td>
            </tr>
        <?php endif; ?>
        <?php if (!$is_www) : ?>
            <tr>
