<?php
/**
 * Defines constants and global variables that can be overridden, generally in app/config/config.yml.
 *
 * @package WordPress
...
function ms_subdomain_constants()
{
    ...
    if ($subdomain_error) {
        ...
        /* translators: 1: VHOST, 2: SUBDOMAIN_INSTALL, 3: app/config/config.yml, 4: is_subdomain_install() */
            ...
            '<code>VHOST</code>',
            '<code>multisite.subdomain_install</code>',
            '<code>app/config/config.yml</code>',
            '<code>is_subdomain_install()</code>'
        );
        if ($subdomain_error_warn) {
            trigger_error(
                __('<strong>Conflicting values for the constants VHOST and SUBDOMAIN_INSTALL.</strong> The value of SUBDOMAIN_INSTALL will be assumed to be your subdomain configuration setting.') . ' ' . $vhost_deprecated,
                E_USER_WARNING
            );
        } else {
