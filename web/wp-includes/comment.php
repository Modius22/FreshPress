    ...
    if (1 == get_option('comment_whitelist')) {
        if ('trackback' != $comment_type && 'pingback' != $comment_type && $author != '' && $email != '') {
            ...
            if (!empty($comment_user->ID)) {
                $ok_to_comment = $wpdb->get_var($wpdb->prepare(
...
function get_lastcommentmodified($timezone = 'server')
{
    ...
    switch ($timezone) {
        ...
        case 'server':
            ...
            $comment_modified_date = $wpdb->get_var($wpdb->prepare(
 ...
 * @return Error|true Error when a comment field exceeds the limit,
 *                       otherwise true.
 */
function wp_check_comment_data_max_lengths($comment_data)
{
    ...
    if (isset($comment_data['comment_author']) && mb_strlen(
    ...
    if (isset($comment_data['comment_content']) && mb_strlen(
 ...
 * that it is properly set, such as in app/config/config.yml, for your environment.
 *
...
function pingback($content, $post_id)
{
    ...
    foreach ((array)$post_links_temp as $link_test) :
        if (!in_array(
            $link_test,
                $pung
        ) && (url_to_postid($link_test) != $post->ID) // If we haven't pung it already and it isn't a link to itself
...
function _prime_comment_caches($comment_ids, $update_meta_cache = true)
{
    ...
    if (!empty($non_cached_ids)) {
        $fresh_comments = $wpdb->get_results(sprintf(
...
function wp_handle_comment_submission($comment_data)
{
    ...
    if (!comments_open($comment_post_ID)) {
        ...
        if (current_user_can('read_post', $comment_post_ID)) {
            return new Error('comment_on_draft', __('Sorry, comments are not allowed for this item.'), 403);
        } else {
    ...
    if ($user->exists()) {
        ...
        if (get_option('comment_registration')) {
            return new Error('not_logged_in', __('Sorry, you must be logged in to comment.'), 403);
        }
    }
    ...
    if (get_option('require_name_email') && !$user->exists()) {
        if ('' == $comment_author_email || '' == $comment_author) {
            return new Error(
                'require_name_email',
                __('<strong>ERROR</strong>: please fill the required fields (name, email).'),
                200
            );
        } elseif (!is_email($comment_author_email)) {
            return new Error(
                'require_valid_email',
                __('<strong>ERROR</strong>: please enter a valid email address.'),
                200
            );
        }
    }
