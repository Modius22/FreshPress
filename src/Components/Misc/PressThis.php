...
class PressThis
{
    ...
    public function save_post()
    {
        if (empty($_POST['post_ID']) || !$post_id = (int)$_POST['post_ID']) {
            wp_send_json_error(['errorMessage' => __('Missing post ID.')]);
        }

        if (empty($_POST['_wpnonce']) || !wp_verify_nonce($_POST['_wpnonce'], 'update-post_' . $post_id) ||
            !current_user_can('edit_post', $post_id)) {
            wp_send_json_error(['errorMessage' => __('Invalid post.')]);
        }
        ...
        if (current_user_can($category_tax->cap->assign_terms)) {
            $post_data['post_category'] = (!empty($_POST['post_category'])) ? $_POST['post_category'] : [];
        }
        ...
        if (is_wp_error($updated)) {
            wp_send_json_error(['errorMessage' => $updated->get_error_message()]);
        } else {
            ...
            if ($redirect) {
                wp_send_json_success(['redirect' => $redirect, 'force' => $forceRedirect]);
            } else {
                wp_send_json_success(['postSaved' => true]);
            }
        }
    }
    ...
    public function add_category()
    {
        ...
        foreach ($names as $cat_name) {
            ...
            if (term_exists($cat_name, $taxonomy->name, $parent)) {
                if (count($names) === 1) {
                    wp_send_json_error(['errorMessage' => __('This category already exists.')]);
                } else {
                    continue;
                }
            }
        ...
        if (empty($added)) {
            wp_send_json_error(['errorMessage' => __('This category cannot be added. Please change the name and try again.')]);
        }
        ...
        foreach ($added as $new_cat_id) {
            ...
            if (is_wp_error($new_cat)) {
                wp_send_json_error(['errorMessage' => __('Error while adding the category. Please try again later.')]);
            }
    ...
    public function fetch_source_html($url)
    {
        ...
        $remote_url = wp_safe_remote_get($url, [
            'timeout' => 30,
    ...
    private function _limit_img($src)
    {
        ...
        if (preg_match('!/ad[sx]?/!i', $src)) {
            ...
            if (preg_match('!(/share-?this[^.]+?\.[a-z0-9]{3,4})(\?.*)?$!i', $src)) {
                // Share-this type button
                return '';
            } else {
                if (preg_match('!/(spinner|loading|spacer|blank|rss)\.(gif|jpg|png)!i', $src)) {
                ...
                if (preg_match('!//vimeo\.com/moogaloop\.swf\?clip_id=([\d]+)$!i', $src, $src_matches)) {
                    ...
                    if (preg_match('!//vine\.co/v/([^/]+)/embed!i', $src, $src_matches)) {
                        ...
                        if (preg_match(
                            ...
                            if (!$oembed->get_provider($src, ['discover' => false])) {
                                $src = '';
                            }
                        }
                    }
    ...
    public function source_data_fetch_fallback($url, $data = [])
    {
        ...
        if (is_wp_error($source_content)) {
            return ['errors' => $source_content->get_error_messages()];
        }
    ...
    public function merge_or_fetch_data()
    {
        ...
        if (apply_filters('enable_press_this_media_discovery', true)) {
            ...
            if (empty($_POST) && !empty($data['u'])) {
                ...
                foreach (['_images', '_embeds'] as $type) {
                    if (empty($_POST[$type])) {
                        continue;
                    }
                ...
                foreach (['_meta', '_links'] as $type) {
                    ...
                    foreach ($items as $key => $value) {
                        ...
                        if ($type === '_meta') {
                            ...
                            if (in_array($key, ['canonical', 'shortlink', 'icon'], true)) {
    ...
    public function categories_html($post)
    {
        ...
        <div aria-label="<?php esc_attr_e('Categories'); ?>">
            <ul class="categories-select">
                <?php wp_terms_checklist($post->ID, ['taxonomy' => 'category', 'list_only' => true]); ?>
            </ul>
        </div>
        <?php
    }
    ...
    public function tags_html($post)
    {
        ...
        if ($user_can_assign_terms) {
            ?>
            <button type="button" class="button-link tagcloud-link" id="link-post_tag"
    ...
    public function get_source_site_name($data)
    {
        ...
        if (!empty($data['_meta'])) {
            if (!empty($data['_meta']['og:site_name'])) {
                ...
                if (!empty($data['_meta']['application-name'])) {
        ...
        if (!empty($data['s'])) {
            ...
            if (!empty($data['_meta'])) {
                if (!empty($data['_meta']['twitter:description'])) {
                    ...
                    if (!empty($data['_meta']['og:description'])) {
                        $text = $data['_meta']['og:description'];
                    } else {
                        if (!empty($data['_meta']['description'])) {
    ...
    public function html()
    {
        ...
        add_filter('mce_css', [$this, 'add_editor_style']);
            ...
            <body class="wp-admin wp-core-ui <?php echo $admin_body_classes . ' ' . $admin_body_class; ?>">
                <div id="adminbar" class="adminbar">
                    <h1 id="current-site" class="current-site">
                        <a class="current-site-link" href="<?php echo esc_url(home_url('/')); ?>" target="_blank"
                           rel="home">
                ...
                <div id="scanbar" class="scan">
                    <form method="GET">
                        ...
                        <input type="url" name="u" id="url-scan" class="scan-url"
                               value="<?php echo esc_attr($site_data['u']) ?>"
        ...
        wp_nonce_field('add-category', '_ajax_nonce-add-category', false); ?>
                    ...
                    <div class="wrapper">
                        <div class="editor-wrapper">
                            <div class="alerts" role="alert" aria-live="assertive" aria-relevant="all"
                                 aria-atomic="true">
                                <?php
                            ...
                            <div id="app-container" class="editor">
                                ...
                                <?php
                                wp_editor($post_content, 'pressthis', [
                        ...
                        <div class="post-actions">
                            ...
                            <div class="split-button">
                                <div class="split-button-head">
                                    <button type="button" class="publish-button split-button-primary"
                                            aria-live="polite">
