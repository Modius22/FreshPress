...
class Manager
{
            ...
            foreach ($attachments as $symbol => $attachment) {
                ...
                if (isset($existing_starter_content_posts['attachment:' . $attachment['post_name']])) {
                    ...
                    if (empty($attached_file) || !file_exists($attached_file)) {
                        $attachment_id = null;
                        $attached_file = null;
                    } elseif ($this->get_stylesheet() !== get_post_meta(
                            $attachment_post->ID,
        ...
        foreach ($nav_menus as $nav_menu_location => $nav_menu) {
            ...
            foreach ($nav_menu['items'] as $nav_menu_item) {
                ...
                if (isset($nav_menu_item['object_id'])) {
                    if ('post_type' === $nav_menu_item['type'] && preg_match(
                            '/^{{(?P<symbol>.+)}}$/',
    ...
    public function save()
    {
        ...
        if (isset($_POST['customize_changeset_status'])) {
            ...
            if (!get_post_status_object($changeset_status) || !in_array(
                    $changeset_status,
    ...
    public function save_changeset_post($args = [])
    {
        ...
        if ('publish' === $args['status'] && false === has_action(
        ...
        if ($update_transactionally && $invalid_setting_count > 0) {
            $response = [
                ...
                'message' => sprintf(_n(
                    'There is %s invalid setting.',
                    'There are %s invalid settings.',
    ...
    public function register_controls()
    {
        ...
        $this->add_section(new ThemesSection($this, 'themes', [
        ...
        if (!$this->is_theme_active()) {
            $themes = wp_prepare_themes_for_js([wp_get_theme($this->original_stylesheet)]);
            $active_theme = current($themes);
            $active_theme['isActiveTheme'] = true;
            $this->add_control(new ThemeControl($this, $active_theme['id'], [
                'theme' => $active_theme,
                'section' => 'themes',
        ...
        foreach ($themes as $theme) {
            ...
            $this->add_control(new ThemeControl($this, $theme_id, [
                'theme' => $theme,
                'section' => 'themes',
                'settings' => 'active_theme',
            ]));
        ...
        $this->add_control(new SiteIconControl($this, 'site_icon', [
            'label' => __('Site Icon'),
            'description' => sprintf(
        ...
        $this->add_control(new CroppedImageControl($this, 'custom_logo', [
            'label' => __('Logo'),
            'section' => 'title_tagline',
            'priority' => 8,
        ...
        $this->add_control(new ColorControl($this, 'header_textcolor', [
        ...
        $this->add_control(new ColorControl($this, 'background_color', [
        ...
        $this->add_setting(new HeaderImageSetting($this, 'header_image_data', [
            'theme_supports' => 'custom-header',
        ]));
        ...
        $this->add_control(new MediaControl($this, 'header_video', [
            'theme_supports' => ['custom-header', 'video'],
        ...
        $this->add_setting(new BackgroundImageSetting($this, 'background_image_thumb', [
            'theme_supports' => 'custom-background',
            'sanitize_callback' => [$this, '_sanitize_background_setting'],
        ]));
        ...
        $this->add_control(new BackgroundPositionControl($this, 'background_position', [
        ...
        $this->add_section('custom_css', [
            ...
            'description' => sprintf(
                '%s<br /><a href="%s" class="external-link" target="_blank">%s<span class="screen-reader-text">%s</span></a>',
    ...
    public function _validate_header_video($validity, $value)
    {
        ...
        if ($video) {
            ...
            if ('.mp4' !== substr($video, -4) && '.mov' !== substr(
                    $video,
                    -4
