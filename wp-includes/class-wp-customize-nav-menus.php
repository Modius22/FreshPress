...
final class WP_Customize_Nav_Menus
{
    ...
    public function customize_register()
    {
        ...
        if (current_theme_supports('widgets')) {
            ...
            $description .= '<p>' . sprintf(__('Menus can be displayed in locations defined by your theme or in <a href="%s">widget areas</a> by adding a &#8220;Custom Menu&#8221; widget.'),
                    "javascript:wp.customize.panel( 'widgets' ).focus();") . '</p>';
        } else {
        ...
        if (1 == $num_locations) {
            ...
            $description = '<p>' . sprintf(_n('Your theme supports %s menu. Select which menu appears in each location.',
                    'Your theme supports %s menus. Select which menu appears in each location.', $num_locations),
                    number_format_i18n($num_locations)) . '</p>';
        }
        if (current_theme_supports('widgets')) {
            /* translators: URL to the widgets panel of the customizer */
            $description .= '<p>' . sprintf(__('You can also place menus in <a href="%s">widget areas</a> with the &#8220;Custom Menu&#8221; widget.'),
                    "javascript:wp.customize.panel( 'widgets' ).focus();") . '</p>';
        }
        ...
        foreach ($menus as $menu) {
            ...
            foreach (array_values($menu_items) as $i => $item) {
                ...
                $this->manager->add_setting(new WP_Customize_Nav_Menu_Item_Setting($this->manager,
                    $menu_item_setting_id, array(
                        'value' => $value,
                        'transport' => 'postMessage',
                    )));

                // Create a control for each menu item.
                $this->manager->add_control(new WP_Customize_Nav_Menu_Item_Control($this->manager,
                    $menu_item_setting_id, array(
                        'label' => $item->title,
                        'section' => $section_id,
                        'priority' => 10 + $i,
    ...
    public function ajax_insert_auto_draft_post()
    {
        ...
        if (is_wp_error($r)) {
            ...
            $data = array(
                ...
                'message' => sprintf(__('%1$s could not be created: %2$s'), $singular_name,
    ...
    public function sanitize_nav_menus_created_posts($value)
    {
        ...
        foreach (wp_parse_id_list($value) as $post_id) {
            ...
            if (!current_user_can($post_type_obj->cap->publish_posts) || !current_user_can($post_type_obj->cap->edit_post,
                    $post_id)) {
                continue;
            }
            $post_ids[] = $post->ID;
    ...
    public function filter_wp_nav_menu($nav_menu_content, $args)
    {
        if (isset($args->customize_preview_nav_menus_args['can_partial_refresh']) && $args->customize_preview_nav_menus_args['can_partial_refresh']) {
            $attributes = sprintf(' data-customize-partial-id="%s"',
            ...
            $attributes .= sprintf(' data-customize-partial-placement-context="%s"',
