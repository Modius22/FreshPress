...
class Requests_Transport_fsockopen implements Requests_Transport
{
    ...
    public function request($url, $headers = array(), $data = array(), $options = array())
    {
        ...
        $socket = stream_socket_client($remote_socket, $errno, $errstr, ceil($options['connect_timeout']),
    ...
    public function request_multiple($requests, $options)
    {
        ...
        foreach ($requests as $id => $request) {
            try {
                $handler = new $class();
                $responses[$id] = $handler->request($request['url'], $request['headers'], $request['data'],
                    $request['options']);

                $request['options']['hooks']->dispatch('transport.internal.parse_response',
                    array(&$responses[$id], $request));
