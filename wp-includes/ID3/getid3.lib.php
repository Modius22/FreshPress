...
class getid3_lib
{
    ...
    public static function Float2String($floatvalue, $bits)
    {
        ...
        return self::BigEndian2String(self::Bin2Dec($signbit . $exponentbitstring . $fractionbitstring), $bits % 8,
            false);
    }
    ...
    public static function BigEndian2Float($byteword)
    {
        ...
        if (($exponent == (pow(2, $exponentbits) - 1)) && ($fraction != 0)) {
            ...
            $floatvalue = pow(2,
                    ($exponent - (pow(2, $exponentbits - 1) - 1))) * (1 + self::DecimalBinary2Float($fractionstring));
            if ($signbit == '1') {
                $floatvalue *= -1;
            }
        }
    ...
    public static function Dec2Bin($number)
    {
        ...
        for ($i = 0; $i < count($bytes); $i++) {
            $binstring = (($i == count($bytes) - 1) ? decbin($bytes[$i]) : str_pad(decbin($bytes[$i]), 8, '0',
    ...
    public static function PlaytimeString($seconds)
    {
        ...
        return $sign . ($H ? $H . ':' : '') . ($H ? str_pad($M, 2, '0', STR_PAD_LEFT) : intval($M)) . ':' . str_pad($S,
                2, 0, STR_PAD_LEFT);
    }
    ...
    {
        return self::BigEndian2Int(substr($rawdata, 0, 1)) + (float)(self::BigEndian2Int(substr($rawdata, 1,
                    1)) / pow(2, 8));
    }
    ...
    {
        return self::BigEndian2Int(substr($rawdata, 0, 2)) + (float)(self::BigEndian2Int(substr($rawdata, 2,
                    2)) / pow(2, 16));
    }
    ...
    public static function FixedPoint2_30($rawdata)
    {
        ...
        return self::Bin2Dec(substr($binarystring, 0, 2)) + (float)(self::Bin2Dec(substr($binarystring, 2, 30)) / pow(2,
                    30));
    }
    ...
    public static function CreateDeepArray($ArrayPath, $Separator, $Value)
    {
        ...
        if (($pos = strpos($ArrayPath, $Separator)) !== false) {
            $ReturnedArray[substr($ArrayPath, 0, $pos)] = self::CreateDeepArray(substr($ArrayPath, $pos + 1),
                $Separator, $Value);
        } else {
    ...
    public static function hash_data($file, $offset, $end, $algorithm)
    {
        ...
        while (true) {
            if (GETID3_OS_ISWINDOWS) {
                ...
                $commandline = GETID3_HELPERAPPSDIR . 'head.exe -c ' . $end . ' ' . escapeshellarg(str_replace('/',
                        ...
                        if (!empty($value)) {
                            if (empty($ThisFileInfo['comments'][$tagname])) {
                                ...
                                foreach ($ThisFileInfo['comments'][$tagname] as $existingkey => $existingvalue) {
                                    ...
                                    if (($newvaluelength <= $oldvaluelength) && (substr($existingvalue, 0,
                                ...
                                foreach ($ThisFileInfo['comments'][$tagname] as $existingkey => $existingvalue) {
                                    $oldvaluelength = strlen(trim($existingvalue));
                                    if ((strlen($existingvalue) > 10) && ($newvaluelength > $oldvaluelength) && (substr(trim($value),
                                                0, strlen($existingvalue)) == $existingvalue)) {
