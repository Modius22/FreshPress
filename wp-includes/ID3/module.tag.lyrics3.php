...
class getid3_lyrics3 extends getid3_handler
{
    ...
    public function Analyze()
    {
        ...
        if ($lyrics3end == 'LYRICSEND') {
            ...
            $lyrics3size = strrev(substr(strrev($lyrics3_id3v1), 9,
                    6)) + 6 + strlen('LYRICS200'); // LSZ = lyrics + 'LYRICSBEGIN'; add 6-byte size field; add 'LYRICS200'
            $lyrics3offset = $info['filesize'] - $lyrics3size;
            $lyrics3version = 2;
    ...
    public function getLyrics3Data($endoffset, $version, $length)
    {
        ...
        if (substr($rawdata, 0, 11) != 'LYRICSBEGIN') {
            if (strpos($rawdata, 'LYRICSBEGIN') !== false) {
                ...
                $info['warning'][] = '"LYRICSBEGIN" expected at ' . $endoffset . ' but actually found at ' . ($endoffset + strpos($rawdata,
                            'LYRICSBEGIN')) . ' - this is invalid for Lyrics3 v' . $version;
                ...
                $info['error'][] = '"LYRICSBEGIN" expected at ' . $endoffset . ' but found "' . substr($rawdata, 0,
                        11) . '" instead';
                return false;
        ...
        switch ($version) {
            ...
            case 1:
                if (substr($rawdata, strlen($rawdata) - 9, 9) == 'LYRICSEND') {
                    ...
                    $info['error'][] = '"LYRICSEND" expected at ' . ($this->ftell() - 11 + $length - 9) . ' but found "' . substr($rawdata,
                            strlen($rawdata) - 9, 9) . '" instead';
                    return false;
                }
                break;
            ...
            case 2:
                if (substr($rawdata, strlen($rawdata) - 9, 9) == 'LYRICS200') {
                    $ParsedLyrics3['raw']['unparsed'] = substr($rawdata, 11,
                        strlen($rawdata) - 11 - 9 - 6); // LYRICSBEGIN + LYRICS200 + LSZ
                    ...
                    if (isset($ParsedLyrics3['raw']['IND'])) {
                        ...
                        foreach ($flagnames as $flagname) {
                            if (strlen($ParsedLyrics3['raw']['IND']) > $i++) {
                                $ParsedLyrics3['flags'][$flagname] = $this->IntString2Bool(substr($ParsedLyrics3['raw']['IND'],
                                    $i, 1 - 1));
                            }
                        }
                    }
                    ...
                    $info['error'][] = '"LYRICS200" expected at ' . ($this->ftell() - 11 + $length - 9) . ' but found "' . substr($rawdata,
                            strlen($rawdata) - 9, 9) . '" instead';
                    return false;
                }
                break;
