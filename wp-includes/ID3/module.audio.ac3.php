    ...
    public function Analyze()
    {
        ...
        if (strpos($this->AC3header['syncinfo'], self::syncword) === 0) {
            ...
            if (!$this->isDependencyFor('matroska')) {
                ...
                return $this->error('Expecting "' . getid3_lib::PrintHexBytes(self::syncword) . '" at offset ' . $info['avdataoffset'] . ', found "' . getid3_lib::PrintHexBytes(substr($this->AC3header['syncinfo'],
                        0, 2)) . '"');
            }
            $offset = 0;
        ...
        $thisfile_ac3_raw['synchinfo']['crc1'] = getid3_lib::LittleEndian2Int(substr($this->AC3header['syncinfo'],
            $offset, 2));
        $ac3_synchinfo_fscod_frmsizecod = getid3_lib::LittleEndian2Int(substr($this->AC3header['syncinfo'],
            ($offset + 2), 1));
        ...
        $thisfile_ac3['frame_length'] = self::frameSizeLookup($thisfile_ac3_raw['synchinfo']['frmsizecod'],
        ...
        $thisfile_ac3['service_type'] = self::serviceTypeLookup($thisfile_ac3_raw_bsi['bsmod'],
        ...
        $thisfile_ac3['channels_enabled'] = self::channelsEnabledLookup($thisfile_ac3_raw_bsi['acmod'],
        ...
        if ($thisfile_ac3_raw_bsi['addbsi_flag']) {
            ...
            $thisfile_ac3_raw_bsi['addbsi_data'] = substr($this->AC3header['bsi'], $this->BSIoffset,
