...
class getID3
{
    ...
    public function __construct()
    {
        ...
        } elseif (preg_match('#([0-9]+)G#i', $this->memory_limit,
    ...
    public function openfile($filename)
    {
        try {
            ...
            $filename = preg_replace('#(.+)' . preg_quote(DIRECTORY_SEPARATOR) . '{2,}#U', '\1' . DIRECTORY_SEPARATOR,
                $filename);

            // open local file
            //if (is_readable($filename) && is_file($filename) && ($this->fp = fopen($filename, 'rb'))) { // see http://www.getid3.org/phpBB3/viewtopic.php?t=1720
            if ((is_readable($filename) || file_exists($filename)) && is_file($filename) && ($this->fp = fopen($filename,
                    'rb'))) {
                // great
            } else {
            ...
            if ($this->option_max_2gb_check) {
                ...
                if (($fseek < 0) || (($this->info['filesize'] != 0) && (ftell($this->fp) == 0)) ||
                    ...
                    if ($real_filesize === false) {
                        ...
                        unset($this->info['filesize']);
                        fclose($this->fp);
                        throw new getid3_exception('PHP seems to think the file is larger than ' . round(PHP_INT_MAX / 1073741824) . 'GB, but filesystem reports it as ' . number_format($real_filesize,
                                3) . 'GB, please report to info@getid3.org');
                    }
                    $this->info['filesize'] = $real_filesize;
                    $this->warning('File is larger than ' . round(PHP_INT_MAX / 1073741824) . 'GB (filesystem reports it as ' . number_format($real_filesize,
        ...
        try {
            ...
            if (!$this->option_tag_id3v2) {
                ...
                if ((substr($header, 0, 3) == 'ID3') && (strlen($header) == 10)) {
                    ...
                    $this->info['avdataoffset'] += getid3_lib::BigEndian2Int(substr($header, 6, 4),
            ...
            if (isset($determined_format['fail_id3']) && (in_array('id3v1', $this->info['tags']) || in_array('id3v2',
                        $this->info['tags']))) {
            ...
            if (!empty($this->info[$comment_name]['comments'])) {
                ...
                if ($this->option_tags_html) {
                    foreach ($this->info['tags'][$tag_name] as $tag_key => $valuearray) {
                        $this->info['tags_html'][$tag_name][$tag_key] = getid3_lib::recursiveMultiByteCharString2HTML($valuearray,
                            $encoding);
                    }
                }
    ...
    public function getHashdata($algorithm)
    {
        ...
        if (!empty($this->info['fileformat']) && !empty($this->info['dataformat']) && ($this->info['fileformat'] == 'ogg') && ($this->info['audio']['dataformat'] == 'vorbis')) {
            ...
            if (!empty($this->info['avdataoffset']) || (isset($this->info['avdataend']) && ($this->info['avdataend'] < $this->info['filesize']))) {
                ...
                $this->info[$algorithm . '_data'] = getid3_lib::hash_data($this->info['filenamepath'],
                    $this->info['avdataoffset'], $this->info['avdataend'], $algorithm);
    ...
    public function saveAttachment($name, $offset, $length, $image_mime = null)
    {
        try {
            ...
            if ($this->getid3->option_save_attachments === getID3::ATTACHMENTS_NONE) {
                ...
                $dir = rtrim(str_replace(array('/', '\\'), DIRECTORY_SEPARATOR, $this->getid3->option_save_attachments),
                    DIRECTORY_SEPARATOR);
